...
function ajaxHeart(event) {
  ...
  _axios2.default.post(this.action).then(function (res) {
    var isHearted = _this.heart.classList.toggle('heart__button--hearted'); // means form.heart (name)
    (0, _bling.$)('.heart-count').textContent = res.data.hearts.length;
    if (isHearted) {
      _this.heart.classList.add('heart__button--float');
      setTimeout(function () {
        return _this.hear.classList.remove('heart__button--float');
      }, 2500);
    }
  }).catch(function (err) {
...
var storeLink = function storeLink(store) {
  return '\n<a href="/stores/' + store.slug + '" class="search__result"><strong>' + store.name + '</strong></a>\n';
};
var searchResultHTML = function searchResultHTML(stores) {
  return stores.map(storeLink).join('');
};
...
function typeAhead(searchBox) {
  if (!searchBox) {
    ...
    var resultContainer = searchBox.querySelector('.search__results');
    ...
    input.on('input', function () {
      ...
      if (!this.value) {
        ...
        _axios2.default.get('/api/search', { params: { q: this.value } }).then(function (res) {
          if (res.data.length > 0) {
            resultContainer.innerHTML = _dompurify2.default.sanitize(searchResultHTML(res.data));
          } else {
            resultContainer.innerHTML = _dompurify2.default.sanitize('\n              <div class="search__result">\n               Nothing was found for ' + _this.value + '.\n              </div>\n              ');
          }
        }).catch(function (err) {
          return console.log(err);
        });
...
var _autoCompleteSearch = __webpack_require__(10);
